...
// FIX: Removed AtCoder from the list
const PLATFORMS = [
...
const Home = ({ onNavigate }) => {
  ...
  // Fetch Top 10 for Mini Leaderboard
  useEffect(() => {
    fetch('http://localhost:5000/api/leaderboard')
      .then(res => res.json())
      .then(data => {
        // FIX: Ensure data is an array before slicing and handle potential empty array
        const coders = Array.isArray(data) ? data : (data.leaderboard && Array.isArray(data.leaderboard) ? data.leaderboard : []);
        setTopCoders(coders.slice(0, 10)); // Top 10
        setLoading(false);
      })
      .catch(e => {
        console.error("Error fetching leaderboard data:", e);
        setTopCoders([]); // Set to empty array on failure
        setLoading(false);
      });
  }, []);
  ...
  const RankBadge = ({ index }) => {
    const rank = index + 1;
    let classes = "w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shadow-md";
    let content = `#${rank}`;

    if (rank === 1) {
        classes += " bg-yellow-400 text-yellow-900 border-2 border-yellow-500";
        content = "🥇";
    } else if (rank === 2) {
        classes += " bg-gray-300 text-gray-800 border-2 border-gray-400";
        content = "🥈";
    } else if (rank === 3) {
        classes += " bg-orange-400 text-orange-900 border-2 border-orange-500";
        content = "🥉";
    } else {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-800">
      ...
      {/* --- 1. HERO SECTION (Professional Look) --- */}
      {/* FIX: Using a professional indigo-to-purple gradient for better UX/UI trust */}
      <div className="relative bg-gradient-to-r from-indigo-800 to-purple-900 text-white py-24 px-4 overflow-hidden">
        <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>
        
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          <h1 className="text-4xl md:text-6xl font-extrabold mb-6 tracking-tight leading-tight">
            Elevate Your Competitive <br/> <span className="text-purple-300">Programming Journey</span>
          </h1>
          <p className="text-lg md:text-xl text-indigo-200 mb-8 max-w-3xl mx-auto leading-relaxed">
            CodeCampus helps ITM students monitor their progress across multiple coding platforms 
            and sharpen their problem-solving skills with detailed, real-time analytics.
          </p>
          <button 
            onClick={() => onNavigate('dashboard')}
            className="bg-white text-indigo-900 px-8 py-3 rounded-full font-bold text-lg hover:bg-indigo-50 transition shadow-xl transform hover:scale-105"
          >
            Go to Dashboard
          </button>
        </div>
      ...
      {/* --- 2. WHY USE CODECAMPUS? (Icons fixed with standard emojis) --- */}
      ...
      {/* --- 3. SUPPORTED PLATFORMS (AtCoder removed) --- */}
      <div className="bg-gray-50 py-16 px-4 text-center">
          ...
          <p className="text-gray-500 max-w-2xl mx-auto mb-10 text-sm">
            CodeCampus integrates with major competitive programming platforms.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-8 md:gap-16 items-center max-w-4xl mx-auto opacity-80 grayscale hover:grayscale-0 transition-all duration-500">
              {/* FIX: AtCoder is removed by updating the PLATFORMS array */}
              {PLATFORMS.map((p, i) => (
      ...
      {/* --- 4. ITM CODING LEADERBOARD (Top 10, New Style) --- */}
      <div className="bg-white py-20 px-4">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold text-center text-gray-900 mb-2">ITM Coding Leaderboard</h2>
            <p className="text-center text-gray-500 mb-12">
                {loading ? "Loading top performers..." : topCoders.length > 0 
                ? `Our top ${topCoders.length} performers across all coding platforms.` 
                : "No performers found. Please check your API data."}
            </p>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                ...
                {topCoders.map((student, index) => (
                    // FIX: Implemented requested leaderboard table structure/style
                    <div key={index} className="grid grid-cols-12 items-center p-4 border-b border-gray-100 hover:bg-indigo-50 transition group">
                        ...
                        <div className="col-span-4 flex items-center gap-3">
                            ...
                            <div className="flex flex-col min-w-0">
                                <span className="font-bold text-gray-800 truncate group-hover:text-indigo-600 transition">{student.name}</span>
                                {/* Assuming 'role' helps distinguish */}
                                <span className="text-xs text-gray-500">{student.role === 'alumni' ? 'Alumni' : `Sem ${student.semester}, Year ${student.year}`}</span> 
                            </div>
                        </div>

                        {/* Enrollment */}
                        <div className="col-span-3 text-sm text-gray-500 font-medium">
                            {student.enrollment || `ITM${new Date().getFullYear()}${student.name.substring(0,3).toUpperCase()}`}
                        </div>

                        {/* Branch */}
                        <div className="col-span-2 text-center text-sm">
                            <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-xs font-bold border border-blue-200">
                                {student.branch || 'IT/CSE'}
                            </span>
                        </div>
